...
<style>
    div.stButton > button:first-child { 
        height: 3rem; width: 100%; border-radius: 12px; 
...
if page == "Check-In":
    ...
    # FIXED: Hardened Date Logic
    now = datetime.datetime.now()
    today = st.date_input("Date", now.date())
    ...
    if mode == "Grace Day":
        ...
        if today > month_start:
            # FIXED: Hardened date range logic
            date_range = pd.date_range(start=month_start, end=today - datetime.timedelta(days=1)).date
...
# --- PAGE 2: SCOREBOARD ---
    ...
    if today_obj > month_start:
        active_range = [d.date() for d in pd.date_range(start=month_start, end=today_obj - datetime.timedelta(days=1)) if d.strftime('%A') != 'Sunday']
    else:
        active_range = []
