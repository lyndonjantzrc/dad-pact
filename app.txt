...
st.markdown("""<style>
    div.stButton > button:first-child { height: 3rem; width: 100%; border-radius: 12px; background-color: #D32F2F; color: white; font-weight: bold; }
    .stMetric { background: #f8f9fa; border: 1px solid #eee; padding: 15px; border-radius: 15px; }
...
if not df.empty: df['log_date'] = pd.to_datetime(df['log_date']).dt.date
...
if page == "Check-In":
    ...
    if mode == "Grace Day":
        ...
        # Sunday Logic: Check for missed days to unlock 2nd bonus
        ...
        missed = sum(1 for d in date_range if d not in u_logs['log_date'].tolist() and d.strftime('%A') != 'Sunday')
...
# --- PAGE 2: SCOREBOARD (AUTO-PENALTY ENGINE) ---
elif page == "Scoreboard":
    st.header("🏆 Monthly Standings")
    active_range = [d.date() for d in pd.date_range(start=datetime.now().replace(day=1), end=datetime.now()) if d.strftime('%A') != 'Sunday']
    summary = []
    for d in ["Damien", "Jesse", "Lyndon", "Todd"]:
        ...
        # Auto-Penalty: -15 for every missing Mon-Sat
        missed_pts = sum(1 for day in active_range if day not in d_logs['log_date'].tolist()) * -15
        # 3-Week Streak Bonus (+20): 18 days of 25+ pts
        consist = 20 if len(d_logs[(d_logs['entry_type'] == 'exercise') & (d_logs['points'] >= 25)]) >= 18 else 0
        summary.append({"Dad": d, "Base": logged_pts, "Penalty": missed_pts, "Bonus": consist, "Total": logged_pts + missed_pts + consist})
    
    st.table(pd.DataFrame(summary).sort_values(by="Total", ascending=False).reset_index(drop=True))
...
# --- PAGE 3: CHART ---
    ...
    if not df.empty:
        ...
        st.plotly_chart(px.line(df_sort, x='log_date', y='Cumulative Points', color='participant_name', markers=True))

# --- PAGE 4: ADMIN ---
elif page == "Admin":
    if user == "Lyndon" and st.button("WIPE DATA FOR NEW MONTH"):
